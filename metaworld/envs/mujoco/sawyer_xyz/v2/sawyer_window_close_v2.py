...
class SawyerWindowCloseEnvV2(SawyerXYZEnv):
    ...
    def compute_reward(self, actions, obs):
        if self.reward_func_version == "v2":
            ...
            action_penalty = -0.1  # Negative as we want to penalize large actions

            # Calculate the distance between the robot's end-effector and the window handle
            ee_to_handle_dist = np.linalg.norm(obs[:3] - obs[4:7])

            # Assuming the goal state of the handle is to match the `goal_position`

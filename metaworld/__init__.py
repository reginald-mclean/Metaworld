...
class MT25(Benchmark):
    ...
    def __init__(self, seed=None):
        super().__init__()
        self._train_classes = _env_dict.MT25_V2
        train_kwargs = _env_dict.MT25_V2_ARGS_KWARGS
        self._train_tasks = _make_tasks(
            self._train_classes, train_kwargs, _MT_OVERRIDE, seed=seed
        )
...
class ML25(Benchmark):
    ...
    def __init__(self, seed=None):
        super().__init__()
        self._train_classes = _env_dict.ML25_V2["train"]
        self._test_classes = _env_dict.ML25_V2["test"]
...
def make_mt_envs(
    ...
    **kwargs,
    ...
    benchmark: Benchmark
    if name in ALL_V2_ENVIRONMENTS.keys():
        ...
        raise ValueError(
            "Invalid MT env name. Must either be a valid Metaworld task name (e.g. 'reach-V2'), 'MT10' or 'MT50'."
        )
...
def make_ml_envs(
    ...
    if name in ALL_V2_ENVIRONMENTS.keys():
        ...
        raise ValueError(
            "Invalid ML env name. Must either be a valid Metaworld task name (e.g. 'reach-V2'), 'ML10', 'ML25', or 'ML45'."
        )
...
def register_mw_envs() -> None:
    ...
    register(
        ...
        entry_point=lambda env_name, seed: ALL_V2_ENVIRONMENTS_GOAL_HIDDEN[env_name + "-goal-hidden" if "-goal-hidden" not in env_name else env_name](  # type: ignore
            seed=seed,
        ),
        kwargs={},
    )
    ...
    register(
        ...
        entry_point=lambda env_name, seed: ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE[env_name + "-goal-observable" if "-goal-observable" not in env_name else env_name](  # type: ignore
            seed=seed
        ),
        kwargs={},
    )
